 ...
 * *vocabulary_random*, a simulated random variable having the same length, mean, and standard deviation with the variable *vocabulary*. It'll be treated as the "true" student performance on the test because this sample is randomly selected for the population in which the study was conducted.
 ...
 * 3.1.1. Using the same type of plot you used for *vocabulary*, plot the distribution of *vocabulary_random*  and compare the two plots. You may choose one of the two ways to compare plots: (a) use base R code to put the two plots side-by-side, or (b) use {ggplot2} package to put the two into one plot. 

 * 3.1.2. What are the statistical meanings of the new plot? Compare these two distributions and write one sentence to describe what you have observed. Circle back to question 2.1.2, do you think now whether vocabulary is normally distributed? 
