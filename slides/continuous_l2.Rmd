...
To distinguish population statistics from sample statistics, it is conventional to use separate notation.
...
The mean of our sampling distribution (mean of means) approximates the <span style="color: blue;"> population mean $({\mu})$</span>. This is an example of the **central limit theorem**.
...
---
# One Sample T-Test
* Does **not** test the probability of obtaining the sample distribution or data itself, only the sample mean.
...
* The t-statistic is functionally like a z-score, but for the t-distribution.
* To calculate a t-statistic,replace the population standard deviation with the sample standard deviation.
...
A one vs. two tailed test apply different null hypotheses.
...
* Dr. DSM has developed a new intervention for depression.
...
Suppose we have two populations of students - <span style="color: red;"> students with infrequent school attendance </span> and <span style="color: blue;"> students who attend school regularly </span>. On our standardized test, students with infrequent attendance have an average score of 95 $(\mu_x = 95)$ and students with regular attendance have an average score of 104 $(\mu_y = 104)$. Thus, our true difference in means is 9.
```{r}
...
If we do not have homogeneity of variance, then we would conduct a Welch's t-test.
---
# Estimating Variance
...
$$\hat{\sigma} = \sqrt{\frac{(n_1-1)\hat{\sigma}^2 + (n_2-1)\hat{\sigma}^2 }{n_1+n_2-2}} $$
