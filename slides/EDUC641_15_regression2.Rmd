---
title: "Examining Relationships of Continuous Variables"
...
i_am("slides/EDUC641_15_regression2.rmd")
...
What is the probability that we would have gotten a slope value of 2.23 (or a more extreme value) by sampling from a population in which there was **no relationship** between $LIFE\_EXPECTANCY$ and $SCHOOLING$?
...
.large[.blue[**What do you want people to know about the nature of the variables in your data?**]]
...
.small[Things people (probably) **don't** need to know: number of unique values, summary stats on ID variables, ?]
...
### A descriptive table (Table 1):
```{r, echo=T}
names(who1) <- c("Region", "Status", "Schooling (Yrs)", 
                                  "Life Expectancy (Yrs)")
datasummary_skim(who1,
...
> Need to `install.packages("pandoc")` first
...
```{r, echo=T}
# Tell R to cut by a given variable
datasummary_balance(`Schooling (Yrs)`+ `Life Expectancy (Yrs)` ~ Status,
                    data = who1)
```

--

.blue[***Can you imagine when this might be an especially useful set of descriptive statistics to produce?***]
...
.large[.blue[**Based on what you know so far, what do people need to know about your regression results?**]]
...
### Assignments
- Quiz #5 due **November 27** at 5pm
- Assignment #4 due **December 2** at 11:59PM
- Final assignment due **December 11** at 4:59PM

### No lab on 11/27 or 11/28! Lab replaces class on 11/26!
