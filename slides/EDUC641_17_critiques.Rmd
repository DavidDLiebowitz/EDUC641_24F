...
- Articulate modern quantitatively-based critiques of the null-hypothesis significance testing (NHST) framework
...
- Even in WEIRD countries, many samples are not representative of the national population
 ...
 + Non-Hispanic, White & male patients included in clinical trials more
...
1. Replication work often seen as un-original; weak incentives to conduct replications. 
2. Many replications fail:
.small[
- New contexts
...
---
## Multiple hypothesis testing dangers
...
.red[*Other approaches exist! Bonferroni is an extremely conservative one--beware!*]
]
...
---
## Other data/analysis challenges
...
.small[Never **EVER** use Excel to read in your data]
```{r, echo=F}
include_graphics("gene_name.png")
```

--
.small[More importantly, without being able to observe researcher decisions in their code, we can't assess the reasonableness of choices and the potential for replication. .red-pink[**We need to learn how to code and not just point-and-click for Open Science.**]]

--
.small[Developing a consistent, reproducible [data management workflow](../data_management.html) is key]
...
.purple[**Only D on the previous slide is correct, yet all of these are commonly expressed in the scientific literature!**]
...
--
- .small[Many of these concerns relate to different ways of thinking about probability...]
...
- Asking insurance customers to sign an [honesty pledge](https://www.nytimes.com/2023/06/24/business/economy/francesca-gino-harvard-dishonesty.html) at the beginning (compared to the end) of a policy review form led to more truthful reporting of driving habits $(p<0.001)$ (*a real, very contested paper*)
...
.large[
1. If you're going to translate research into practice or policy, you actually do need to make a binary decision
    + Yes/No we should do this thing; this clinical practice is (is not) better than this one
    + Once you port the Bayesian framework over into this binary decision world, many of the same NHST concerns re-surface
2. Selecting a prior can be **subjective**
3. Difficulty in finding informative prior
4. Disputes over value of uninformative priors
5. Similarity in results across approaches
6. Stasis
]
...
### Re-submission
Assignments <13, resubmissions due on Canvas 12/6 at 5pm
