...
pacman::p_load(tidyverse, here, knitr, blogdown, rio, gifski, palmerpenguins, xaringanthemer)
...
.small[*Note*: These next few classes are focused on concepts rather than programming, so focus your energies there.]
...
---
# Empirical Rule
We can use our understanding of the normal distribution to calculate the probability of observing a value in a certain range. This is known as the .red-pink[**empirical rule**].
...
* The .red-pink[**standard normal distribution**] has a mean of 0 and a standard deviation of 1.
...
.purple[Given these few facts about normal distributions, we are now ready to move on to understand how and why **statistical inference** is possible...!]
...
* Suppose population mean national life expectancy is 71.64 $(\mu)$ with a SD of 8.15 $(\sigma)$.
...
The proportion of the area under the curve above a z-score of 0.53 is just under 0.30 ( $\approx$ 0.296). Therefore, there is a 30 percent probability of randomly selecting a country with a national life expectancy greater than 76.
...
Get ready to have your mind blown by the beauty of the .red-pink[.large[**Central Limit Theorem**]]!
...
* Here is a histogram of the means of 1000 random samples from our WHO data.
 ...
 + The standard deviation is the .red-pink[**standard error of the mean (SEM)**].
]
...
.footnote[[1] *We are going to walk through the steps in class. Just looking at the slides is not sufficient for you to understand how to do this.*]
...
---
# $t$-distributions
* $t$-distributions assume that the *population* standard deviation $(\sigma)$ is unknown. Instead, it uses the *sample* standard deviation $(s)$. 
* $t$-distributions with smaller sample sizes have "fatter" tails. Assumes more uncertainty with smaller sample sizes.
* $t$-distributions resemble $z$-distributions as .red-pink[**degrees of freedom**] increase.

---
# Degrees of freedom

The .red-pink[**degrees of freedom**] refers to the number of values in the calculation of a statistic that are free to vary. 
...
but, when we calculate this in reality, we are using the .red-pink[**sample mean**] $(\bar{x})$ which is constructed **using each $x_i$**!
...
You can look up the .red-pink[**critical *t*-value**] for any number of degrees of freedom and alpha-threshold in any number of $t$-statistic tables you can find [online](https://www.sjsu.edu/faculty/gerstman/StatPrimer/t-table.pdf).
